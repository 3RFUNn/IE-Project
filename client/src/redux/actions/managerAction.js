...
export const createNewDepartment = (req) => {
  return async (dispatch) => {
    const request = async () => {
      const response = await axios.post(`${baseUri}admin/department`, req, {
    ...
    const request = async () => {
      if (req?.userType === "itmanager") {
        const response = await axios.get(
          `${baseUri}admin/managers?page=${req?.page}&limit=${req?.limit}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
        ...
        const response = await axios.get(
          `${baseUri}managers?page=${req?.page}&limit=${req?.limit}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
