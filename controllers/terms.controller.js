import { getCoursePreRegistrationsForTerm, getCoursesPreRegistrationsForTerm, getCoursesRegistrationsForTerm } from "./courses.action.js";
import {
    addTerm,
    editTerm,
    deleteTerm,
...
export const getCoursesPreRegistrationsForTermHandler = async (req, res) => {
    try {
        ...
        if (user) {
            const page = Number(req.query.page ?? 0);
            const limit = Number(req.query.limit ?? 0);
            const preregistrations = await getCoursesPreRegistrationsForTerm(req.params.id);
            res.status(200)
                .json({
