  ...
  try {
    ...
    if (user) {
      ...
      res
        ...
        .json({
          data: term.courses
            .filter((course) => course.__t === "course" || course.__t === "")
...
export const addCourseToTermHandler = async (req, res) => {
  try {
    ...
    if (user.__t === "edumanager") {
      const result = await addCourseToTerm(req.params.id, req.body.courseId);
      if (!result) {
        res
          .status(404)
          .json({
...
export const addCoursesToTermHandler = async (req, res) => {
  try {
    const user = await getUser(req.authData.id);
    if (user.__t === "edumanager") {
      const result = await addCoursesToTerm(req.params.id, req.body.courseIds);
      if (!result) {
        res
          .status(404)
          .json({
...
export const removeCourseFromTermHandler = async (req, res) => {
  try {
    const result = await removeCourseFromTerm(req.params.id, req.body.courseId);
    if (!result) {
      res
        .status(404)
        .json({
  ...
  try {
    ...
    if (user) {
      ...
      res
        .status(200)
        .json({
          data: preregistrations.slice(page * limit, (page + 1) * limit),
          status: 200,
          error: null,
          ok: true,
          message: "",
