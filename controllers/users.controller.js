  ...
  try {
    if (
      ...
      res
        .status(200)
        .json({
          data: students?.slice(page * limit, (page + 1) * limit),
          status: 200,
          error: null,
          ok: true,
          message: "",
  ...
  try {
    if (
      ...
      res
        .status(200)
        .json({
          data: professors.slice(page * limit, (page + 1) * limit),
          status: 200,
          error: null,
          ok: true,
          message: "",
...
export const editProfessorHandler = async (req, res) => {
  try {
    if (
      ...
      const professorEdited = await editProfessor(req?.params?.id, req.body);
      res
        .status(200)
        .json({
